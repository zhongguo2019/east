...
function addReportRule(){
	var df=document.reportFormatForm;	
	df.action='<c:out value="${hostPrefix}" /><c:url value="/reportrule.do" />?method=enterPriRule';
	df.submit();
}
    ...
    <td width="100%" background="<c:url value="${imgPath}/f02.gif"/>" height="36">
      <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tr>
          ...
          <td> 
				<p style="margin-top: 3"><font class=b color="#000000"><b><fmt:message key="view.blgzpz"/>>><fmt:message key="view.yssjgzpz"/></b></font></p>     
          </td>
		  <td></td> 
        </tr>
      </table>
				...
				<td align="center">				
				    <div align="center" style=" width:98%">
						...
						<display:table name="reporRule" cellspacing="0" cellpadding="0"  
								...
								<a href="reportrule.do?method=editThePriRule&repid=<c:out value="${rt.modelid }"/>&rtarid=<c:out value="${rt.targetid }"/>&targetN=<c:out value="${rt.targetname }"/>&rtype=<c:out value="${rt.rtype }"/>&cflag=1&bflag=0&levelFlag=<c:out value="${levelFlag }"/>&rulecode=<c:out value="${rt.rulecode }"/>" ><fmt:message key="view.pz"/></a>
