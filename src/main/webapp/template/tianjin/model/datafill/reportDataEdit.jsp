...
<%-- <script src="<c:url value='/common/jqueryValidata/<c:out value="${filename} "/>'  />"
	type="text/javascript"></script> --%>
          ...
          <td>
							<p style="margin-top: 3">
								<font class=b><b> <%-- <fmt:message
											key="navigation.dataEdit.dataEdit" /> --%> 
											<c:choose>
								<c:when test="${pageFlag==1 }">
									<font class=b><b><fmt:message key="navigation.dataEdit.dataEdit1" /> </b> </font>
								</c:when>
								<c:when test="${pageFlag==2 }">
									<font class=b><b><fmt:message key="navigation.dataEdit.dataEdit2" /> </b> </font>
								</c:when>
								<c:otherwise>
									<font class=b><b><fmt:message key="navigation.dataEdit.dataEdit" /> </b> </font>
								</c:otherwise>
							</c:choose>
											</b> </font>
							</p>
			...
			<td width="100%" bgcolor="#EEEEEE" valign="top"><br>
				...
				value="<c:out value='${singleOrMore}'/>" />  
						...
						<tr>
							<td class="TdBGColor1" width="100%" height="20" align="left"
								valign=middle nowrap>&nbsp;&nbsp;<fmt:message
									key="standard.dataFill.beginDate" />: <input align="top"
								type="hidden" name="showret" value="yes" /> <input
								id="beginDate" style="height: 19; valign: top; width: 100;"
								name="beginDate" type="text"
								value="<c:out value='${beginDate}'/>" readonly="readonly" /> <fmt:message
									key="standard.dataFill.endDate" />: <input type="hidden"
								name="showret" value="yes" /> <input id="endDate"
								style="height: 19; valign: top; width: 100;" name="endDate"
								type="text" value="<c:out value='${endDate}'/>"
								readonly="readonly" /> <fmt:message
									key="standard.dataFill.selectStatus" />: <select
								id="dataStatus" style="height: 19; valign: top; width: 100;"
								name="dataStatus">
									<option value="">
										<fmt:message key="system.switch.hint" />
									</option>
									<option value="0" <c:if test="${state eq '0'}">selected</c:if>>
										<fmt:message key="standard.reportdata.neverChecked" />
									</option>
									<option value="1" <c:if test="${state eq '1'}">selected</c:if>>
										<fmt:message key="standard.reportdata.passed" />
									</option>
									<option value="2" <c:if test="${state eq '2'}">selected</c:if>>
										<fmt:message key="standard.reportdata.notpass" />
									</option>
									<option value="3" <c:if test="${state eq '3'}">selected</c:if>>
										<fmt:message key="standard.reportdata.updated" />
									</option>
							</select> <input type="button" class="btn" id="querydata"
								value="<fmt:message key="standard.dataFill.querydata"/>" />
							</td>
						</tr>
					</table>
					 -->
							...
							<td align="left" class="FormBottom" height="30">&nbsp;&nbsp;
								<c:if test="${openMode=='_self'}">
									...
									<input id="btnReturn" name="btnReturn" class="btn"
										...
										onclick="window.location.href='reportView.do?method=enterInputSearch1' " />
									<!-- 	<fmt:message key="datagather.label.xlsFile" />
					<input type="file" class="file" name="importXlsFile"
							style="width:450" />
						<input type="button" value="<fmt:message key="button.copy"/>"
							style="width:80;height:22;" onclick="xlsCopy();"> -->
					...
					<table width="99%" align="center" cellpadding="1" cellspacing="1"
						class="table">
						<%-- 	<tr>
					<td class="TdBGColor2" align="left"><c:if
						test="${totalPageNum > 1}">
						...
						<c:if test="${page.totalRecord>1 }">
							<tr align="left">
								<td align="left" valign="top" class="TdBGColor2">当前第 <c:out
										value="${page.pageNo}" /> 页 <a
									href="dataFill.do?method=editReportDataEntry&canEdit=1&levelFlag=<c:out value='${levelFlag}'/>&openmode=_self&dataDate=<c:out value='${dataDate}'/>&reportId=<c:out value='${reportId}'/>&organId=<c:out value='${organId}'/>&page=1">首页</a>
