    ...
    <td width="100%" background="<c:url value="${imgPath}/f02.gif"/>" height="36">
      <table border="0" cellpadding="0" cellspacing="0" width="100%">
        <tr>
          ...
          <td>
				<p style="margin-top: 3"><font class=b><b><fmt:message key="navigation.dataFill.dataFill1"/></b></font></p>
          </td>
		  <td></td> 
        </tr>
      </table>
  ...
  <tr>
	...
	<html:form action="reportView.do?method=inputExtraCondition" method="post" onsubmit="return reportCheck()" >
		...
		<tr>
			...
			<tr>
				<td align="right" nowrap class="TdBgColor1"><fmt:message key="standard.dataFill.templateName"/></td>
				<td class="TdBgColor2">
					<slsint:ActiveXTree left="220" top="325" width="380" height="${activeXTreeHeight }"
			      		xml="${reportTreeURL}&levelFlag=0"
			      		bgcolor="0xFFD3C0" 
			      		rootid="10000" 
			      		columntitle="${colName}" 
					...
					<input type="button" value="<fmt:message key="standard.dataFill.importData"/>" style="width:80;" onclick="importDataFromXls('_self','yes')">
						...
						<%-- <select id="xl">
							<option selected="selected" value="1">
								<fmt:message key="select.insert.one" />
							</option>
							<option value="5">
								<fmt:message key="select.insert.five" />
							</option>
							<option value="10">
								<fmt:message key="select.insert.ten" />
							</option>
							<option value="20">
								<fmt:message key="select.insert.twenty" />
							</option>
							<option value="50">
								<fmt:message key="select.insert.fivity" />
							</option>
						</select> --%>
					</td>
			</tr>
...
function importDataFromXls(tg,sr){
	...
	df.action="dataFill.do?method=importDataFromXls&reportId="+vReportId+"&organId="+df.organId.value+"&dataDate="+df.dataDate.value;
...
function addDataOne(tg,sr){
	...
	df.action="dataFill.do?method=editReportDataEntryForDataInput&openmode="+tg+"&canEdit=<c:out value='${canEdit}'/>&levelFlag=0";
	<c:if test="${unitCodeFlag == 1}">
	setUnitCode();
	</c:if>
	if(tg==="_blank"){
