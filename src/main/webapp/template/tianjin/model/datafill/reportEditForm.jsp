...
<c:set var="reportTreeURL">
	<c:out value="${hostPrefix}"/><c:url value='/reportView.do?method=repTreeAjax${params}&levelFlag=2'/>
</c:set>
        ...
        <tr>
          ...
          <td>
            <c:if test="${param.method=='enterViewSearch'}">
				<p style="margin-top: 3"><font class=b><b><fmt:message key="navigation.reportmanage.search"/></b></font></p>
			</c:if>
          	<c:if test="${param.method!='enterViewSearch'}">
          	 <c:if test="${param.levelFlag==2&&param.queryFlag!=1 }">
				<p style="margin-top: 3"><font class=b><b><fmt:message key="navigation.dataEdit.dataEdit"/></b></font></p>
          	 </c:if>
          	 <c:if test="${param.levelFlag==2&&param.queryFlag==1 }">
				<p style="margin-top: 3"><font class=b><b><fmt:message key="navigation.dataEdit.dataselect"/></b></font></p>
          	 </c:if>
          	  <c:if test="${param.levelFlag!=2 }">
				<p style="margin-top: 3"><font class=b><b><fmt:message key="navigation.dataprocess.edit"/></b></font></p>
				</c:if>
			</c:if>
          </td>
		  <td></td> 
...
function viewReport(tg,sr){
	...
	df.action="dataFill.do?method=editReportDataEntry&openmode="+tg+"&canEdit=<c:out value='${canEdit}'/>&levelFlag=2";
	<c:if test="${unitCodeFlag == 1}">
	setUnitCode();
	</c:if>
	if(tg==="_blank"){
