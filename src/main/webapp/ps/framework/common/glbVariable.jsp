...
<c:set var="goldReportNamenVervion"><%=com.krm.ps.util.FuncConfig.getGoldReport()%></c:set>
...
<c:set var="adminltePath">/ps/framework/adminlte</c:set>
<c:set var="displaytag12Path">/ps/framework/displaytag-1.2/css</c:set>
...
<script type="text/javascript">
	...
	var log4javascript_disabled = <%= SysConfig.LOG4JAVASCRIPT_DISABLED %>;
...
<script type="text/javascript">
	...
	var gl_hostPrefix = '<c:out value="${hostPrefix}"/>';
	var gl_systemName = '<c:out value="${systemName}"/>';
	var url_organTree = gl_hostPrefix+'/'+gl_systemName+'/treeAction.do?method=getOrganTreeXML&treeId=';
	<c:if test="${ALERT_MESSAGE != null}">
		alert("<c:out value='${ALERT_MESSAGE}'/>");
	</c:if>
    ...
    function textAreaChange(obj){
        ...
        area_val = area_val.replace(/[']/g,"''"); //替换所有英文单引号为双引号
