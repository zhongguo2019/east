...
function dhtmlXGridObject(id){
 ...
 this.entCnt.rows[1].cells[0].childNodes[0].style.height =(gridHeight - zheight)+"px";
}
};
 ...
 try{}catch(er){
 alert("Error parsing XML: "+er.description)
}
 
 this.objBox.style.height = parseInt(this.entBox.getAttribute("height"))-parseInt(this.hdr.offsetHeight);
 ...
 var elRowIndex = this.rowsCol._dhx_find(el.parentElement)
 ...
 if(this.cell.parentNode.className=="rowselected")
 ...
 if(!arow)aind=this.row.childNodes.length-1;
}
 ...
 if(!this.editor)
{
 ...
 this.parseXML(this.xmlFileUrl,this.rowsCol.length)
}
}else
 return false;
}
