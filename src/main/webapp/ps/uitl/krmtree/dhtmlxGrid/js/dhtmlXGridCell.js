...
function eXcell_co(cell){
 ...
 z++;
 if(this.list.rows[z]){
 this.list.rows[this.i_sel||0].childNodes[0].style.cssText= "";
 this.list.rows[z].childNodes[0].style.cssText= "color:white;background-color:navy";
 this.i_sel=z;
 this.obj.value=this.list.rows[z].childNodes[0].innerHTML;
 this.val=this.obj.value;
}
}
...
(this.obj.runtimeStyle||this.obj.style).cssText = objCssText;
 ...
 var listCssText = "width:"+this.cell.offsetWidth+"px;height:100px;overflow-y:scroll;position:absolute;border:1px solid;border-color:black silver silver black;background-color:white;";
 this.listBox.style.cssText = listCssText
 this.listBox.style.left = arPos[0]-this.grid.objBox.scrollLeft+"px";
 this.listBox.style.top = arPos[1]+this.cell.offsetHeight-this.grid.objBox.scrollTop+"px";
 ...
 this.list.style.cssText = "font-family:arial;font-size:12px;width:100%;table-layout:fixed;cursor:default;";
 this.list.onclick = function(e){
 var ev = e||window.event;
 var cell = ev.target||ev.srcElement
 var tbl = cell.parentNode.parentNode.parentNode;
 
 tbl.editor_obj.setValue(cell.combo_val)
 ...
 cell.combo_val = comboKeys[i];
 cell.innerHTML = val;
 if(comboKeys[i]==this.val){
 cell.style.cssText = "color:white;background-color:navy";
 this.i_sel=i;
 fl = true;
}
}
...
eXcell_co.prototype.setValue = function(val){
 ...
 val=null
 var text = this.grid.getCombo(this.cell._cellIndex).get(val)|| val;
