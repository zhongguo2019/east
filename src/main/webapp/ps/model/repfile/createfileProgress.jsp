...
<DIV id="progressDiv" align="center" style="display:none">
	...
	<DIV id=pcc>
		<DIV id=bpc style="text-align:left">
			<DIV id=bproc><fmt:message key="repfile.create.creating"/></DIV>
		</DIV>
		<DIV id=pc>
		...
		<DIV id=buildInfo style="margin-top:30px;text-align:left">
			<dl>
				...
				<dt><fmt:message key="repfile.create.success"/></dt><dd><span id="created" class="count"></span></dd>
			</dl>
		</DIV>
...
function observePro(){	
	...
	var url='<c:out value="${hostPrefix}" /><c:url value="/repFileAction.do" />?method=createFilePercent&_='+(new Date()).getMilliseconds();
...
function suc(request){
	...
	if(resp.isfinished){
		...
		$("created").innerText=resp.created
	}
	if(resp.isfinished && resp.isfinished == "true"){
		...
		setRate((resp.created/resp.total * 100).toFixed(2), pc, true);
	}
}
...
function dynEff(){
	...
	$("bproc").innerText='<fmt:message key="repfile.create.creating"/>'+ds
}
