...
function  listReportRule(){
	...
	df.action='<c:out value="${hostPrefix}" /><c:url value="/repFileAction.do" />?method=listRepJhgz';
	df.submit();
}

function  listReportRuleOne(pkid,displaypage){
	var df=document.reportFormatForm;	
	df.action='<c:out value="${hostPrefix}" /><c:url value="/repFileAction.do" />?method=listRepJhgzOne&urlpkid='+pkid+'&displaypage='+displaypage;
	df.submit();
}
...
function add(pkid){
	 ...
	 var df=document.reportFormatForm;
	 df.action='<c:out value="${hostPrefix}" /><c:url value="/repFileAction.do" />?method=listRepJhgzAdd';
	 df.submit();
}
...
function exportData(){
	...
	var df=document.reportFormatForm;	
	df.action='<c:out value="${hostPrefix}" /><c:url value="/repFileAction.do" />?method=exportTemplate';
	df.submit();
}
...
function exportDataPDf(){
	$("#organ_name").val($("#combotreeOrganTree").combotree("getText"));
	var df=document.reportFormatForm;	
	df.action='<c:out value="${hostPrefix}" /><c:url value="/repFileAction.do" />?method=exportTemplatePdf';
	df.submit();
}
...
function importDataFromXls(tg,sr){
	 ...
	 url="repFileAction.do?method=importDataFromXls" ;
...
<div name="optionbar"  class="navbar">
					...
					<table >
						<tr>
						     <td  align="right"><fmt:message key="view.databatch"/>:
							 </td>
							 <td>
						...
						<display:table name="repconfig" cellspacing="0" cellpadding="0"  
							   ...
							   <display:column sort="true" 
								...
								<display:column sort="true" headerClass="sortable" width="5%"  titleKey="reportrule.edit">	
								  <c:if test="${rt.errornumber != '0' && rt.errornumber !=null && rt.errornumber !='null'}">   
