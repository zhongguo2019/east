...
<DIV id="progressDiv" align="center" style="display:none">
	...
	<DIV id=pcc>
		<DIV id=bpc style="text-align:left">
			<DIV id=bproc><fmt:message key="repfile.download.compressing"/></DIV>
		</DIV>
		<DIV id=pc>
		...
		<DIV id=buildInfo style="margin-top:30px;text-align:left">
			<dl>
				<dt><fmt:message key="repfile.download.compressing.total"/></dt><dd><span id="total" class="count"></span></dd>
				<dt><fmt:message key="repfile.download.compressing.success"/></dt><dd><span id="compressed" class="count"></span></dd>
			</dl>
		</DIV>
...
function observePro(){	
	...
	var url='<c:out value="${hostPrefix}" /><c:url value="/repFileAction.do" />?method=compressPercent&compressPercentId=<%=request.getParameter("compressPercentId") %>&_='+(new Date()).getMilliseconds();
...
function suc(request){
	...
	if(resp.isfinished){
		...
		$("compressed").innerText=resp.compress
	}
	if(resp.isfinished && resp.isfinished == "success"){
		clearInterval(intv);
		setRate(100.0, pc, true)
		$("bproc").style.width="110px";
		var _url = '<c:out value="${hostPrefix}" /><c:url value="/common/download.jsp" />?filePath=' + resp.downFilePath;
		var aa='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="'+ _url +'"><fmt:message key="button.download"/></a>'
		$("bproc").innerHTML='<fmt:message key="databuild.progress.complete"/>'+aa
		pu.stop();
		if(!isDownload){
			isDownload=true;
		...
		clearInterval(intv);
		setRate(0.0, pc, true)
		$("bproc").innerText='<fmt:message key="databuild.progress.failed"/>'
		pu.stop();
...
function dynEff(){
	...
	$("bproc").innerText='<fmt:message key="repfile.download.compressing"/>'+ds
}
