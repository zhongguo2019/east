...
<body onload="forbiddon();" >
     ...
     <html:form action="reportView.do?method=inputExtraCondition" method="post" styleId="aaaa">
								<table  width="100%" class="navbar"  cellSpacing="0"
									...
									<input type="hidden" name="unitCode"
										...
										<td class="TdBGColor1" width="50px"><input type="hidden" name="showret" value="yes"> 
									...
									style="width:80px;" readonly="true" onClick="WdatePicker()" onchange="changedate();" />
	...
	<c:if test="${leveType == 1}"> 
				...
				<!--  <form action="dataFill.do?method=commitDataStatus" method="post" name="form">
					<display:table name="reportList" cellspacing="0" cellpadding="0"  
							...
							<display:column sort="true" headerClass="sortable" width="3%" title="1111">
										<c:if test="${rt[2]==0 && rt[6] == 0}">
											<input type="checkbox" name="check11" value="<c:out value="${rt[1]}" />,<c:out value="${rt[0]}" />,<c:out value="${rt[4]}" />">
												<c:out value="${rt[3]}" />
										</c:if>
								...
								<form name="reportDataForm" action="dataFill.do?method=getDataByCondition"
							    	...
							    	<c:if test="${leveType != 1}"> 	
										...
										%>
											...
											<display:column sort="true" headerClass="sortable" width="20%"
												titleKey="reportrule.edit">
											<c:if test="${rt[5]!=0}">
												<a style="text-decoration: none;" href='dataFill.do?method=getReportGuideDetailed&organId=<c:out value="${rt[1]}" />&reportId=<c:out value="${rt[0]}" />&targetTable=<c:out value="${rt[4]}" />&reportDate=<c:out value="${dataDate }"/>'><fmt:message key="view.xxxx"/></a>&nbsp;&nbsp;								
											</c:if> 
											<c:if test="${rt[2]!=0}">
													<a style="text-decoration: none;" href='dataFill.do?method=editReportDataEntryForDataInput&openmode=_self&canEdit=1&levelFlag=0&organCode=<c:out value="${rt[1]}" />&reportDate=<c:out value="${dataDate }"/>&reportId=<c:out value="${rt[0]}" />&flag2=1&flagtap=<c:out value="${flagtap}" />'><fmt:message key="view.sjbl"/></a>&nbsp;&nbsp;
											</c:if>	
							    	...
							    	<c:if test="${leveType == 1}"> 
										...
										<display:column   headerClass="sortable" width="3%" title="<input type='checkbox' name='checklist' id='checkall'  onclick='checkAll()'>">
										 <c:if test="${rt[2]==0 && rt[6] == 0}"> 
											<input type="checkbox" name="check1"  value="<c:out value="${rt[1]}" />,<c:out value="${rt[0]}" />,<c:out value="${rt[4]}" />">
										 </c:if> 
											
										</display:column>
											<display:column   headerClass="sortable" width="30%"
												titleKey="reportguideorgan">
												<c:out value="${organInfo.full_name}" />
											</display:column>
											<display:column   headerClass="sortable" width="30%"
											...
											<display:column sort="true" headerClass="sortable" width="17%"
												titleKey="reportrule.edit">
										<%-- <c:if test="${rt[5]!=0}">
											...
											<c:if test="${rt[2]==0 && rt[6] == 0}">
												<a style="text-decoration: none;"  href="javascript:commitDataStatus('<c:out value="${rt[1]}" />','<c:out value="${rt[0]}" />','<c:out value="${rt[4]}" />')"><fmt:message key="view.sjsh"/></a>
											</c:if>		
											<c:if test="${rt[2]==0 && rt[6] > 0}">
												<font style="color:green;"><fmt:message key="view.sjysh"/></font>
												<font style="margin-left:10px;"><a style="text-decoration: none;" href="javascript:unLockDataStatus('<c:out value="${rt[1]}" />','<c:out value="${rt[0]}" />')">
...
<script type="text/javascript">
	...
	function viewReport(tg, sr) {
		...
		df.action = "dataFill.do?method=getReportGuide&levelFlag=1&leveType=<c:out value='${leveType}'/>&reportId="+vReportId+"&organCode="+organCode;
	...
	function commitDataStatus(organId, reportId,targetTable){
		...
		var dataDate = $("#dataDate").val();
		var url = "dataFill.do?method=commitDataStatus";
		var param = {"organId":organId, "reportId":reportId,"dataDate" : dataDate,"targettable" : targetTable,"aaaa":0};
		$.ajax({
			url : url,
			data : param,
			type : "post",
