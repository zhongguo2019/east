...
<body topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0" style="overflow-y:hidden ">
	...
	<table border="0" cellpadding="0" cellspacing="0" width="100%" height="70%">
				<html:form action="xlsInit" method="post" enctype="multipart/form-data" styleId="xlsInitForm">
						...
						<tr height="30%">
							<td align="center">
							<a href="dataFill.do?method=exportTemplate&organId=<c:out value='${organId}'/>&reportId=<c:out value='${reportId}'/>&dataDate=<c:out value='${dataDate}'/>"   class="easyui-linkbutton" style="width:80;height: 20;text-decoration: none;" ><fmt:message key="repFile.exp"/></a>
							<a href="#"   onclick="xlsInit();" class="easyui-linkbutton" style="width:80;height: 20;text-decoration: none;" ><fmt:message key="standard.dataFill.importData"/></a>
							</td>
							
							<!-- <input type=file name=j  onchange="ye.value=value">
	...
	<script type="text/javascript">
		...
		function xlsInit() {
			...
			//var xlsfilename = $('#file1').filebox('getText');
			 if(xlsfilename==""){
				alert("<fmt:message key="importexport.file.nothing"/>");
				return false;
			}
			if(validateFile("xls",xlsfilename.substring(xlsfilename.lastIndexOf(".")+1))){
			 	document.getElementById("xlsInitForm").action = "<c:out value="${hostPrefix}"/>/<c:out value="${systemName}"/>/xlsInit.do?method=uploadExcelYJH&xlsfilename="+xlsfilename;
