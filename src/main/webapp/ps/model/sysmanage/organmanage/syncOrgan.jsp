...
function msg()
	{
		...
		if(msg=="success")
			{
				alert("<fmt:message key='jigoutongbuchenggong'/>");
			}
		else if(msg=="fail")
			{
				alert("<fmt:message key='jigoutongbushibai'/>");
			}
 	...
 	function validation()
 	{
		...
		{
			alert("<fmt:message key='qingxuanzeshijian'/>");
			document.runModelDataForm.date.focus();
		    return false;
		}
 	}
 	//window.parent.document.getElementById("ppppp").value= "<fmt:message key="usermanage.sysadmin.menu"/><fmt:message key="navigation.arrowhead"/>";

</script>
<body leftmargin="0" onload="msg()">
	...
	<tr>
  	...
  	<tr>
  		<td align="center">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input type="submit" name="mitsearcher" style="width: 60px;height: 20px;margin: auto;" value="<fmt:message key="view.jgtb"/>" onclick="return validation()">
					 ...
					 <display:column sort="true" titleKey="runmodel.status" headerClass="sortable" width="12%">
						    <c:choose>
						    	<c:when test="${rmd.status==2 }">
								    	<c:out value="<fmt:message key="view.tbcg"/>"/>
						    	</c:when>
						    	<c:when  test="${rmd.status==3 }">
						    			<c:out value="<fmt:message key="view.tbsb"/>"/>
						    	</c:when>
						    </c:choose>
				   </display:column>
				</display:table>
