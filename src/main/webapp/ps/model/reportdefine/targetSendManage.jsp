<!-- /ps/model/reportdefine/targetSendManage. -->
...
/*  function aaaaa(){
	 ...
	 var target2=document.getElementsByName("target2");
	if(document.getElementById("aaaaa").checked==true){
	//alert(target2.length);
	for(var i=0;i<target2.length;i++){
		
		target2[i].checked="checked";
		}
	}else{
		for(var i=0;i<target2.length;i++){
			target2[i].checked="";
...
<body topmargin="0" leftmargin="0" rightmargin="0" bottommargin="0" onload="forbiddon()"class="easyui-layout" style="overflow-y:hidden ">

<div data-options="region:'north',border:false,title:' <fmt:message key="view.bbdy.bswh"/> '" style="width:100%;padding:0px;overflow-y:hidden ">
...
<!--  <input type="checkbox" id="aaaaa"   onclick="aaaaa();">  -->
  ...
  <tr>
	...
	<html:form action="reportView.do?method=inputExtraCondition" method="post" onsubmit="return reportCheck();" >
		...
		<tr>
			...
			<tr>
				...
				<td>
			      	<div id="flagReport12">
					<input id="combotreeRepTree1" class="easyui-combotree"  value="<c:out value='${reportId1}'/>"   style="width:380"/>
				</div>
				<div id="flagReport22">
					<input id="combotreeRepTree1" class="easyui-combotree" value="<c:out value='${reportId1}'/>" multiple style="width:380">
				</div>	
				</td>
				</tr>
				<tr>
	...
	<c:if test="${targetList!=null}">
		...
		<div class="menucontent">
				...
				<table  width="100%">
											<tr>
												<td colspan="3"  valign="top"   align="center"  ><label><font color="#0673DA"><c:if test="${template.name != model.name}"><fmt:message key="view.xzglzd"/></c:if> &nbsp;&nbsp;<c:out value="${template.name}"/></font></label>
...
if((reportId1==""||reportId1 == null) && (vReportId !=""||vReportId != null)){
	...
	df.action="reportTargetAction.do?method=listTarget&openmode="+tg +"&levelFlag=2"+"&reportId="+vReportId+"&reportId1="+reportId1;
	df.target=tg;
	df.submit();
}
