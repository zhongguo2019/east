...
function addReportRule(){
	...
	df.submit(); */
	window.parent.window.OpenPanel('<c:out value="${hostPrefix}" /><c:url value="/reportrule.do" />?method=enterBasicRule&levelFlag=1','600','320','<fmt:message key="view.jbgzpz"/>');
}
...
function changestocktype(val){
	if('812'==val){
	 ...
	 /* window.parent.document.getElementById("ppppp").value= "<fmt:message key="view.blgzpz"/>>><fmt:message key="view.jblgzpz"/>";
 */
...
<div name="optionbar"  class="navbar">
				    ...
				    <div align="center" style=" width:100%">
						...
						<display:table name="reporRule" cellspacing="0" cellpadding="0"  
								...
								<a style="text-decoration: none;" href="#" onclick="window.parent.window.OpenPanel('reportrule.do?method=editTheReportRule&repid=<c:out value="${rt.modelid }"/>&rulecode=<c:out value="${rt.rulecode }"/>&rtarid=<c:out value="${rt.targetid }"/>&targetN=<c:out value="${rt.targetname }"/>&rtype=<c:out value="${rt.rtype }"/>&cflag=1&bflag=0&levelFlag=<c:out value="${levelFlag }"/>&isdetail=0&typename=<c:out value="${rt.typename }"/>&rename=<c:out value="${rt.reportname }"/>','800','500','<fmt:message key="view.ywzbgzpi"/> >> <fmt:message key="view.jbgzpz"/> >> <fmt:message key="view.ckxx"/>')" ><fmt:message key="view.ckxx"/></a>&nbsp;&nbsp;
								<a style="text-decoration: none;" href="#" onclick="window.parent.window.OpenPanel('reportrule.do?method=editTheReportRule&reportName=<c:out value="${rt.reportname}"/>&repid=<c:out value="${rt.modelid }"/>&rtarid=<c:out value="${rt.targetid }"/>&targetN=<c:out value="${rt.targetname }"/>&rtype=<c:out value="${rt.rtype }"/>&cflag=1&bflag=0&levelFlag=<c:out value="${levelFlag }"/>&rulecode=<c:out value="${rt.rulecode }"/>&flags=1','800','500','<fmt:message key="view.ywzbgzpi"/> >> <fmt:message key="view.jbgzpz"/>')" ><fmt:message key="view.pz"/></a>
