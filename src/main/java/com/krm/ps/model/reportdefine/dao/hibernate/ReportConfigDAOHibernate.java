...
public class ReportConfigDAOHibernate extends BaseDAOHibernate implements
	...
	public ReportConfig getReportConfig(Long reportId, int funId, Long idxId) {
		String hql = "from ReportConfig t where t.reportId = " + reportId
				+ " and t.funId = " + funId + " and t.idxId = " + idxId;
		Map map = new HashMap();
	...
	public List getReportConfigs(Long reportId, Long funcId) {
		...
		if (funcId.intValue() == 21 || funcId.intValue() == 22) {
			hql = "from ReportConfig t where t.reportId = " + reportId
					+ " and t.funId in (21,22) " + " order by t.idxId,t.funId";
			...
			if (list.size() > 0) {
				for (Iterator itr = list.iterator(); itr.hasNext();) {
					...
					hql = "from ReportConfig t where t.reportId = " + reportId
							+ " and t.funId in (14,17,18) and idx_id =  "
							+ repConfig.getIdxId()
			...
			hql = "from ReportConfig t where t.reportId = " + reportId
					+ " and t.funId = " + funcId + " order by t.idxId,t.funId";
		}
		Map map = new HashMap();
		if (!hql.equals(""))
	...
	public List getReportConfigs(Long reportId, Long funcId, Long defInt) {
		String hql = "from ReportConfig t where t.reportId = " + reportId
				+ " and t.funId = " + funcId + " and t.defInt = " + defInt;
		Map map = new HashMap();
		return list(hql, map);
	}
	...
	public List getReportConfigs(Long pkId, Long reportId, Long funcId,
		...
		List result = list(sql, new Object[][] { { "t", ReportConfig.class } },
				null, null);
		return result;
	}
	...
	public List getReportConfigsByFunIdAndOrgIdsAndDate(String funId,
			String orgIds) {
		String sql = "select {t.*} from code_rep_config t where t.def_char in ("
				+ orgIds + ")" + " and t.fun_id=" + funId;
		List result = list(sql, new Object[][] { { "t", ReportConfig.class } },
				null, null);
		return result;
	}
	...
	public void saveConfig(ReportConfig config, int isUpdate) {
		if (isUpdate == 0) {
			...
			result = list(sqlStr, null, scalaries,
					new Object[] { config.getReportId(), config.getFunId() });
	...
	public String[] getColumnByRepIdAndFunId(String reportId, String funId) {
		...
		String sql = "select def_int as di from code_rep_config t where t.report_id = "
				+ reportId + " and t.fun_id = " + funId + " order by t.idx_id";
	...
	public void copyConfigs(Long newRepid, Long oldRepid) {
		...
		if ('s' == SysConfig.DB) {
			Sql = "insert into code_rep_config(report_id, fun_id, idx_id,"
					+ "def_int, def_char, description)" + " select " + newRepid
					+ ", fun_id, idx_id," + "def_int, def_char, description "
					+ "from code_rep_config where report_id=" + oldRepid
					+ " and fun_id not in(31,41)";
		} else {
			Sql = "insert into code_rep_config(pkid, report_id, fun_id, idx_id,"
					+ "def_int, def_char, description)"
					+ " select "
					+ DBDialect.genSequence("code_rep_config_seq")
					+ ","
					+ newRepid
					+ ", fun_id, idx_id,"
					+ "def_int, def_char, description "
					+ "from code_rep_config where report_id="
					+ oldRepid
					+ " and fun_id not in(31,41)";
		}
		jdbcUpdate(Sql, null);
	...
	public void removeReportConfig(Long repId, Long funId) {
		String sql = "delete from code_rep_config where report_id =" + repId
				+ " and fun_id = " + funId;
		jdbcUpdate(sql, null);
	}
	...
	public ReportConfig getReportConfig(Long pkid) {
		return (ReportConfig) this.getHibernateTemplate().get(
				ReportConfig.class, pkid);
	}
