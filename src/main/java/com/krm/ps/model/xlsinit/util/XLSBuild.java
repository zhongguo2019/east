...
public class XLSBuild {
	 ...
	 * @throws Exception
	 */
	public static DataSet constructDataTotitle(FormFile in) throws Exception{
		Workbook wb = null;
		Sheet[] sheets = null;
		List data = new ArrayList();
		List title = new ArrayList();
		try {
			...
			int rows = sheets[0].getRows();
			int cols = sheets[0].getColumns();
			//	数据批次	检核规则	检核表名 模板有这三列的才算正常模板
			if("数据批次".equals(sheets[0].getCell(1,1).getContents().trim()) && "检核规则".equals(sheets[0].getCell(2,1).getContents().trim()) && "检核表名".equals(sheets[0].getCell(3,1).getContents().trim())){
				//取得excel数据的标题
				for (int i=0;i<cols;i++){
					title.add(sheets[0].getCell(i,1).getContents().trim());
				}
				//构造excel数据集
